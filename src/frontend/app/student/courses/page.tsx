...
export default function MyCoursesPage() {
  ...
  // Find all archived courses that are bundled with the courses the student is enrolled in.
  const enrolledCourseIds = inProgressCourses.map(c => c.id).concat(completedCourses.map(c => c.id));
  const bundledCourseIds = courses
    .filter(c => enrolledCourseIds.includes(c.id) && c.includedArchivedCourseIds)
    .flatMap(c => c.includedArchivedCourseIds!);
  const uniqueBundledCourseIds = [...new Set(bundledCourseIds)];
  const includedArchivedCourses = courses.filter(c => uniqueBundledCourseIds.includes(c.id));

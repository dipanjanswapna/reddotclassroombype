...
import { HomepageConfig, TeamMember, TopperPageCard, TopperPageSection, WhyChooseUsFeature, Testimonial, OfflineHubHeroSlide, Organization, Instructor, StoreHomepageSection, StoreHomepageHero, StoreHomepageBanner, StoreHomepageProductSection, Product } from '@/lib/types';
...
export default function AdminHomepageManagementPage() {
  ...
  useEffect(() => {
    async function fetchConfig() {
      try {
        const [fetchedConfig, instructorsData, organizationsData, productsData] = await Promise.all([
            getHomepageConfig(),
            getInstructors(),
            getOrganizations(),
            getProducts(),
        ...
        setAllProducts(productsData.filter(p => p.isPublished));
      } catch (error) {
  ...
  const handleStoreHeroChange = (field: keyof StoreHomepageHero, value: any) => {
      setConfig(prev => {
          if (!prev) return null;
          const storeSection = prev.storeHomepageSection || { hero: { title: '', subtitle: '', imageUrl: '' } };
          const newHero = { ...storeSection.hero, [field]: value };
          return { ...prev, storeHomepageSection: { ...storeSection, hero: newHero as StoreHomepageHero } };
      });
  };
  
  const handleFeaturedProductsChange = (productId: string, add: boolean) => {
      setConfig(prev => {
          if (!prev) return null;
          const storeSection = prev.storeHomepageSection || {};
          const currentIds = storeSection.featuredProductIds || [];
          const newIds = add
              ? [...currentIds, productId]
              : currentIds.filter(id => id !== productId);
          return { ...prev, storeHomepageSection: { ...storeSection, featuredProductIds: newIds } };
      });
  };

  const handleStoreProductSectionChange = (index: number, field: keyof StoreHomepageProductSection, value: string) => {
    setConfig(prev => {
        if (!prev) return null;
        const storeSection = prev.storeHomepageSection || {};
        const productSections = [...(storeSection.productSections || [])];
        if (productSections[index]) {
            productSections[index] = { ...productSections[index], [field]: value };
        }
        return { ...prev, storeHomepageSection: { ...storeSection, productSections } };
    });
  };
  
  const addStoreProductSection = () => {
    setConfig(prev => {
        if (!prev) return null;
        const storeSection = prev.storeHomepageSection || {};
        const productSections = [...(storeSection.productSections || []), { title: 'New Section', category: '' }];
        return { ...prev, storeHomepageSection: { ...storeSection, productSections } };
    });
  };

  const removeStoreProductSection = (index: number) => {
    setConfig(prev => {
        if (!prev) return null;
        const storeSection = prev.storeHomepageSection || {};
        const productSections = storeSection.productSections?.filter((_, i) => i !== index);
        return { ...prev, storeHomepageSection: { ...storeSection, productSections } };
    });
  };
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-8">
      ...
      <Tabs defaultValue="general" className="w-full" value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="store" className="mt-6 space-y-8">
            <Card>
                <CardHeader>
                    <CardTitle>Store Hero Section</CardTitle>
                    <CardDescription>Manage the main hero banner on the /store page.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="space-y-2">
                        <Label>Title</Label>
                        <Input value={config.storeHomepageSection?.hero?.title || ''} onChange={(e) => handleStoreHeroChange('title', e.target.value)} />
                    </div>
                    <div className="space-y-2">
                        <Label>Subtitle</Label>
                        <Textarea value={config.storeHomepageSection?.hero?.subtitle || ''} onChange={(e) => handleStoreHeroChange('subtitle', e.target.value)} />
                    </div>
                    <div className="space-y-2">
            ...
            <Card>
             ...
             <Card>
                <CardHeader>
                    ...
                    <CardDescription>Select products to feature on the store homepage.</CardDescription>
                </CardHeader>
                <CardContent>
                     <Popover>
                        <PopoverTrigger asChild>
                            <Button variant="outline" className="w-full justify-start text-left h-auto min-h-10">
                                ...
                                {(config.storeHomepageSection?.featuredProductIds || []).length > 0
                                    ? (config.storeHomepageSection?.featuredProductIds || []).map(id => {
                            ...
                            <Command>
                                ...
                                <CommandGroup>
                                    {allProducts.map(p => (
                                        <CommandItem
                                            key={p.id}
                                            onSelect={() => handleFeaturedProductsChange(p.id!, !(config.storeHomepageSection?.featuredProductIds || []).includes(p.id!))}
                                        >
                                            <Check className={cn("mr-2 h-4 w-4", (config.storeHomepageSection?.featuredProductIds || []).includes(p.id!) ? "opacity-100" : "opacity-0")} />
                                            {p.name}
                                        </CommandItem>
                                    ))}
                                </CommandGroup>
             ...
             <Card>
                <CardHeader>
                    ...
                    <CardDescription>Create sections on the store page to display products by category.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    {config.storeHomepageSection?.productSections?.map((section, index) => (
                        <div key={index} className="p-4 border rounded-md relative space-y-4">
                            <Button variant="ghost" size="icon" className="absolute top-1 right-1" onClick={() => removeStoreProductSection(index)}><X className="text-destructive h-4 w-4"/></Button>
                            <div className="space-y-2">
                                <Label>Section Title</Label>
                                <Input value={section.title} onChange={(e) => handleStoreProductSectionChange(index, 'title', e.target.value)} />
                            </div>
                            <div className="space-y-2">
                                <Label>Product Category</Label>
                                <Select value={section.category} onValueChange={(v) => handleStoreProductSectionChange(index, 'category', v)}>
                    ...
                    <Button variant="outline" className="w-full" onClick={addStoreProductSection}><PlusCircle className="mr-2"/>Add Product Section</Button>
                </CardContent>
            </Card>
        </TabsContent>

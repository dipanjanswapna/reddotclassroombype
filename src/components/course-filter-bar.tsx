...
export function CourseFilterBar({ categories, subCategories, instructors, providers }: CourseFilterBarProps) {
  ...
  return (
    <Card className="mb-4 rounded-[20px] border-primary/10 shadow-xl overflow-hidden group transition-all duration-300" style={{ backgroundColor: 'rgb(194, 231, 255)' }}>
      <div className="p-5 md:p-8 flex flex-col lg:flex-row gap-8 items-center relative">
            <div className="absolute top-0 left-0 w-1.5 h-full bg-primary/20 group-hover:bg-primary transition-colors" />
            
            <div className="flex items-center gap-4 shrink-0">
                <div className="p-3 bg-white/50 rounded-xl text-primary shadow-inner border border-white/20">
                    <Filter className="h-5 w-5" />
                </div>
                <div className="flex flex-col">
                    <h3 className="font-black text-lg md:text-xl tracking-tighter uppercase leading-none text-gray-900">Filters</h3>
                    <p className="text-[9px] font-black uppercase text-gray-600 tracking-[0.2em] mt-1">Smart Sorting</p>
                </div>
            </div>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 w-full">
                <div className="space-y-2">
                    <Label className="text-[10px] uppercase font-black tracking-[0.15em] text-gray-600 ml-1">Category</Label>
                    <Select value={selectedCategory} onValueChange={(v) => handleSelect('category', v)}>
                        <SelectTrigger className="rounded-xl h-11 bg-white/60 border-primary/5 hover:border-primary/20 transition-all font-bold text-xs"><SelectValue placeholder="All Categories" /></SelectTrigger>
                ...
                <div className="space-y-2">
                    <Label className="text-[10px] uppercase font-black tracking-[0.15em] text-gray-600 ml-1">Subject</Label>
                    <Select value={selectedSubCategory} onValueChange={(v) => handleSelect('subCategory', v)}>
                        <SelectTrigger className="rounded-xl h-11 bg-white/60 border-primary/5 hover:border-primary/20 transition-all font-bold text-xs"><SelectValue placeholder="All Subjects" /></SelectTrigger>
                        <SelectContent className="rounded-xl border-white/10 shadow-2xl">
                ...
                <div className="space-y-2">
                    <Label className="text-[10px] uppercase font-black tracking-[0.15em] text-gray-600 ml-1">Instructor</Label>
                    <Select value={selectedInstructor} onValueChange={(v) => handleSelect('instructor', v)}>
                        <SelectTrigger className="rounded-xl h-11 bg-white/60 border-primary/5 hover:border-primary/20 transition-all font-bold text-xs"><SelectValue placeholder="All Mentors" /></SelectTrigger>
                        <SelectContent className="rounded-xl border-white/10 shadow-2xl">
                ...
                <div className="space-y-2">
                    <Label className="text-[10px] uppercase font-black tracking-[0.15em] text-gray-600 ml-1">Partner</Label>
                    <Select value={selectedProvider} onValueChange={(v) => handleSelect('provider', v)}>
                        <SelectTrigger className="rounded-xl h-11 bg-white/60 border-primary/5 hover:border-primary/20 transition-all font-bold text-xs"><SelectValue placeholder="All Partners" /></SelectTrigger>
                        <SelectContent className="rounded-xl border-white/10 shadow-2xl">

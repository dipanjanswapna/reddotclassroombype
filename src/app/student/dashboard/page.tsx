  ...
  return (
      <div className="space-y-10 md:space-y-14">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="glassmorphism-card border-primary/20 bg-primary/5 shadow-xl rounded-[2rem] overflow-hidden group border-b-4 border-primary">
            ...
            <Card className="glassmorphism-card border-accent/20 bg-accent/5 shadow-xl rounded-[2rem] overflow-hidden group border-b-4 border-accent">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Card className="glassmorphism-card border-yellow-500/20 bg-yellow-500/5 shadow-xl rounded-[2rem] overflow-hidden group border-b-4 border-yellow-500">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <h2 className="font-headline text-2xl font-black uppercase tracking-tight flex items-center gap-4">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {stats.inProgressCourses.length > 0 ? stats.inProgressCourses.map((course: any) => (
                    <Card key={course.id} className="glassmorphism-card flex flex-col border-primary/10 rounded-[2.5rem] shadow-xl overflow-hidden group hover:shadow-2xl transition-all duration-500 bg-card">
                        ...
                        <div className="p-8 pt-0 mt-auto">
                          <Button asChild className="w-full font-black uppercase tracking-widest text-[10px] h-14 rounded-2xl shadow-2xl shadow-primary/20 active:scale-95 transition-all bg-primary hover:bg-primary/90 text-white border-none">
                            <Link href={`/student/my-courses/${course.id}`}>Enter Learning Zone</Link>
                          </Button>
                        </div>
                    </Card>
                )) : (
                    <div className="col-span-full text-center py-24 bg-muted/30 rounded-[3rem] border-4 border-dashed border-primary/5">
                ...
                <ul className="divide-y-2 divide-primary/5">
                  {stats.upcomingDeadlines.length > 0 ? stats.upcomingDeadlines.map((deadline: any, index: number) => (
                    <li key={index} className="flex items-start gap-5 p-8 hover:bg-muted/30 transition-colors">
                      <div className="flex h-14 w-14 items-center justify-center rounded-[1.25rem] bg-primary/10 shrink-0 border-2 border-primary/5 shadow-inner">
                ...
                <ul className="divide-y-2 divide-primary/5">
                  {stats.recentAchievements.length > 0 ? stats.recentAchievements.map((ach: any) => (
                    <li key={ach.id} className="flex items-start gap-5 p-8 hover:bg-muted/30 transition-colors">
                      <div className="flex h-14 w-14 items-center justify-center rounded-[1.25rem] bg-yellow-500/10 shrink-0 border-2 border-yellow-500/5 shadow-inner">

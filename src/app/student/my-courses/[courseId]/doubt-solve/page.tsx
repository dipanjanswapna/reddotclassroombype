...
function DoubtThread({ doubt, answers, onReopen, onSatisfied }: { doubt: Doubt, answers: DoubtAnswer[], onReopen: (doubtId: string, question: string) => void, onSatisfied: (doubtId: string, rating: number) => void }) {
  ...
  return (
    <div className="space-y-4">
      <div className="p-4 bg-muted rounded-[20px] border border-primary/5">
        <p className="text-[8px] font-black uppercase tracking-widest text-primary mb-1">Question</p>
        {doubt.questionText && <p className="text-xs md:text-sm font-medium leading-relaxed whitespace-pre-wrap text-foreground">{doubt.questionText}</p>}
        {doubt.attachments?.[0]?.url && (
            <div className="mt-2 relative aspect-video w-full max-w-[240px] rounded-xl overflow-hidden border border-white shadow-md">
                <Image src={doubt.attachments[0].url} alt="Question attachment" fill className="object-cover" />
            </div>
        )}
        <p className="text-[8px] font-bold text-muted-foreground mt-2 uppercase tracking-tighter">Asked {formatDistanceToNow(safeToDate(doubt.askedAt), { addSuffix: true })}</p>
      </div>
      ...
      <div className="space-y-3">
        {answers.sort((a,b) => safeToDate(a.answeredAt).getTime() - safeToDate(b.answeredAt).getTime()).map(answer => (
            <div key={answer.id} className={cn(
                "p-4 rounded-[20px] border max-w-[90%]",
                ...
                <p className="text-xs md:text-sm font-medium leading-relaxed whitespace-pre-wrap">{answer.answerText}</p>
                <p className="text-[8px] font-bold text-muted-foreground mt-1.5 uppercase tracking-tighter">{formatDistanceToNow(safeToDate(answer.answeredAt), { addSuffix: true })}</p>
            </div>
        ))}
      </div>
      
      {doubt.status === 'Answered' && (
        <div className="p-4 border border-primary/20 rounded-[20px] bg-primary/5 space-y-4">
            <div className="space-y-2">
                 <Label className="font-black uppercase text-[8px] tracking-widest text-foreground">Need more help?</Label>
                 <Textarea 
                    value={followup} 
                    onChange={e => setFollowup(e.target.value)} 
                    placeholder="Type your follow-up..."
                    className="rounded-xl border-primary/10 bg-white h-20 text-sm"
                />
                 <Button onClick={handleReopen} size="sm" className="font-black uppercase text-[8px] tracking-widest h-8 px-4 rounded-lg" disabled={!followup.trim() || isReopening}>
                   {isReopening ? <Loader2 className="mr-1.5 h-3 w-3 animate-spin"/> : <RefreshCw className="mr-1.5 h-3 w-3"/>}
                   Re-open
                </Button>
            </div>
             <div className="border-t border-primary/10 pt-4">
                <Label className="font-black uppercase text-[8px] tracking-widest text-foreground">Rate the solution</Label>
                 ...
                 <Button onClick={handleSatisfied} size="sm" className="mt-3 font-black uppercase text-[8px] tracking-widest h-8 px-4 rounded-lg" variant="accent" disabled={rating === 0 || isSatisfying}>
                    {isSatisfying ? <Loader2 className="mr-1.5 h-3 w-3 animate-spin"/> : <CheckCircle className="mr-1.5 h-3 w-3"/>}
                    Mark Satisfied
                </Button>
            </div>
        </div>
...
export default function DoubtSolvePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <h1 className="font-headline text-2xl font-black uppercase tracking-tight text-foreground">Doubt <span className="text-primary">Solving</span></h1>
        <p className="text-[10px] md:text-sm text-muted-foreground font-medium">Expert help for {course?.title}.</p>
      </motion.div>
      ...
      <Card className="rounded-[20px] border-primary/20 shadow-xl overflow-hidden bg-[#eef2ed] dark:bg-card/40">
        ...
        <CardContent className="p-4 space-y-4">
            <Textarea
                ...
                rows={4}
                className="rounded-xl border-primary/10 bg-white h-24 focus:border-primary/50 text-sm"
            />
            <div className="grid grid-cols-1 gap-4">
                <div className="space-y-1">
                    <Label className="text-[8px] font-black uppercase tracking-widest text-muted-foreground ml-1">Attach an image (Optional)</Label>
                    <ImageUploader onAnswerChange={setNewDoubtImage} existingImageUrl={newDoubtImage || undefined} />
                </div>
                <Button onClick={handleAskDoubt} disabled={isSubmitting || (!newDoubtText.trim() && !newDoubtImage)} className="h-10 rounded-xl font-black uppercase tracking-widest text-xs shadow-lg">
      ...
      <div className="space-y-4">
        <h2 className="font-headline text-base font-black uppercase tracking-tight border-l-4 border-primary pl-3">Previous Discussions</h2>
        {doubts.length > 0 ? (
          <Accordion type="single" collapsible className="w-full space-y-2">
            {doubts.map(doubt => (
              <AccordionItem key={doubt.id} value={doubt.id!} className="border border-primary/10 rounded-[20px] bg-card overflow-hidden shadow-sm">
                <AccordionTrigger className="px-4 py-3 hover:no-underline hover:bg-primary/5 transition-all">
                    <div className="flex-grow text-left flex items-center gap-3 min-w-0">
                        <div className={cn(
                        ...
                        <div className="truncate pr-2">
                            <p className="font-black text-xs uppercase tracking-tight truncate">{doubt.questionText || "Image Inquiry"}</p>
                            <p className="text-[8px] font-bold text-muted-foreground mt-0.5 uppercase tracking-tighter">{formatDistanceToNow(safeToDate(doubt.askedAt), { addSuffix: true })}</p>
                        </div>
                    </div>
                    <Badge variant={getStatusBadgeVariant(doubt.status)} className="ml-1 font-black text-[8px] uppercase tracking-widest px-2 h-5 shrink-0">{doubt.status}</Badge>
                </AccordionTrigger>
                <AccordionContent className="p-4 border-t border-primary/5 bg-muted/10">
          ...
          <div className="text-center py-16 border-2 border-dashed border-primary/10 rounded-[20px] bg-muted/5">
            <MessageSquare className="w-10 h-10 text-muted-foreground mx-auto mb-3 opacity-20" />
            <p className="text-muted-foreground font-black uppercase tracking-widest text-[10px]">No questions found</p>
          </div>
        )}
      </div>
    </div>

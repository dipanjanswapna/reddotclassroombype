        ...
        <div className="bg-gray-900 text-white min-h-screen">
                        ...
                        >
                            ...
                            <div className="relative z-10 w-full max-w-[600px] aspect-[3/2]">
                                <Image
                                    ...
                                    fill
                                    className="object-cover drop-shadow-2xl rounded-[20px] border-4 border-white/5"
            ...
            <section id="programs" className="py-12 md:py-16">
                <div className="container mx-auto px-4">
                    <div className="max-w-2xl text-left border-l-4 border-primary pl-6 mb-12">
                        <h2 className="font-headline text-3xl md:text-4xl font-black tracking-tight uppercase text-left">
                            {offlineHubData?.programsTitle?.[language] || "Available Programs"}
                        </h2>
                        <p className="text-gray-400 font-medium text-lg mt-2 text-left">Pick your path to academic excellence in our physical classrooms.</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-0 md:gap-y-8">
                        {courses.length > 0 ? courses.map((course) => {
                            ...
                            <p className="col-span-full text-center py-20 text-gray-500 font-medium border border-white/5 rounded-[20px] bg-white/5">No offline courses available at the moment.</p>
                        )}
                    </div>
                </div>
            </section>
                ...
                <div className="container mx-auto px-4">
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {branches.length > 0 ? branches.map((branch) => (
                            <Card key={branch.id} className="p-6 border-white/5 bg-white/5 hover:bg-white/10 hover:border-primary/30 transition-all duration-300 group rounded-[20px]">
                                <div className="space-y-4 text-left">
                                    ...
                                    <div>
                                        <h3 className="font-black text-xl leading-tight group-hover:text-primary transition-colors uppercase tracking-tight text-left">{branch.name}</h3>
                                        <p className="text-sm text-gray-500 mt-2 line-clamp-2 leading-relaxed text-left">{branch.address}</p>
                                    </div>
                            ...
                            <p className="col-span-full text-center py-12 text-gray-500 border border-dashed border-white/10 rounded-[20px]">Offline centers are currently being updated. Please check back later.</p>
                        )}
                    </div>
                </div>
            </section>
            ...
            {offlineHubData?.contactSection?.display && (
                <section className="py-12 md:py-16">
                    <div className="container mx-auto px-4">
                        <div className="relative rounded-[30px] overflow-hidden p-8 md:p-16 bg-gradient-to-br from-primary via-primary/80 to-black text-white shadow-2xl">
                            ...
                            <div className="relative z-10 max-w-3xl space-y-6 text-left">
                                ...
                                <div className="pt-6 flex flex-col sm:flex-row items-center gap-4">
                                    <Button asChild size="lg" className="w-full sm:w-auto rounded-xl font-black uppercase tracking-widest h-14 px-10 bg-white text-primary hover:bg-white/90 shadow-xl border-none">

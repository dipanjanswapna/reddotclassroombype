...
import {
  LayoutDashboard, BookCopy, Users, PhoneCall, QrCode, BarChart3, Banknote, Paintbrush, LogOut, Settings, IdCard
} from 'lucide-react';
...
/**
 * @fileOverview Seller Layout refined with px-1 wall-to-wall layout and indigo active pills.
 */
        ...
        <div className="flex justify-start items-center h-full w-full max-w-full overflow-x-auto no-scrollbar scroll-smooth gap-1 px-2">
          {menuItems.map((item) => (
            ...
            >
              {getIsActive(item.href) && (
                <motion.div 
                  layoutId="active-nav-pill-seller-master"
                  className="absolute inset-x-1 inset-y-2 bg-white dark:bg-indigo-600 shadow-[0_4px_15px_rgba(0,0,0,0.1)] rounded-xl -z-10"
                  transition={{ type: "spring", bounce: 0.2, duration: 0.6 }}
                />
              )}
              <item.icon className={cn("w-5 h-5", getIsActive(item.href) ? "text-indigo-600 scale-110" : "scale-100")} />
              <span className={cn(
                "text-[9px] md:text-[10px] font-black uppercase tracking-tight whitespace-nowrap",
                getIsActive(item.href) ? "text-indigo-600" : "text-muted-foreground"
              )}>{item.label}</span>
            </Link>
          ))}
        </div>

...
import { getCourse, getCourses, getCategories, getInstructorByUid } from '@/lib/firebase/firestore';
...
export default function CourseBuilderPage({ params }: { params: { courseId: string } }) {
  ...
  useEffect(() => {
    if (userInfo) {
        getInstructorByUid(userInfo.uid).then(instructor => {
            if (instructor?.organizationId) {
                setOrganizationId(instructor.organizationId);
            }
        });
    }
  }, [userInfo]);

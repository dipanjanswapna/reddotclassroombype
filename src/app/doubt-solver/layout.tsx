...
export default function DoubtSolverLayout({
  ...
  const menuItems = [
    { href: "/doubt-solver/dashboard", icon: LayoutDashboard, label: "Home" },
            ...
            >
              {getIsActive(item.href) && (
                <motion.div 
                  ...
                  className="absolute inset-x-1 inset-y-2 bg-teal-600/10 rounded-xl -z-10"
                  transition={{ type: "spring", bounce: 0.2, duration: 0.6 }}
                />
              )}
              <item.icon className={cn("w-5 h-5", getIsActive(item.href) ? "text-teal-600" : "")} />
